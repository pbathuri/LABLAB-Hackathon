            ...
            <div className="flex items-center justify-around py-2">
                {navItems.map((item) => {
                    ...
                    return (
                        ...
                        >
                            <div className={`relative p-2 rounded-xl transition-all ${isActive ? 'bg-[#00D9FF]/10' : ''
                                }`}>
                                <Icon size={22} />
                                {isActive && (
...
export function MobileDrawer({ isOpen, onClose, currentScreen, onNavigate, onLogout }: MobileDrawerProps) {
    ...
    return (
        <>
            ...
            {/* Drawer */}
            <div
                className={`fixed top-0 left-0 bottom-0 w-80 max-w-[85vw] bg-[#0F1629] z-50 transform transition-transform duration-300 ease-out ${isOpen ? 'translate-x-0' : '-translate-x-full'
                    }`}
            >
                {/* Header */}
                    ...
                    <div className="px-4 space-y-1">
                        {menuItems.map((item) => {
                            ...
                            return (
                                ...
                                >
                                    <div className={`p-2 rounded-lg ${isActive ? 'bg-[#00D9FF]/20 text-[#00D9FF]' : 'bg-[#151C2C] text-[#8892A7]'
                                        }`}>
                                        <Icon size={18} />
                                    </div>
